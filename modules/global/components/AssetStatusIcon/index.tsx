...
const AssetStatusIcon: FunctionComponent<AssetStatusIconProps> = ({
  sensorType,
  status,
}) => {
  if (sensorType === 'energy') {
    return (
      <svg width="12" height="12" viewBox="0 0 9 12" fill="none">
        <path
  ...
  return (
    <span className="h-[18px] w-[18px] inline-flex items-center justify-center">
      <span
        className={`h-[8px] w-[8px] rounded-full ${
          status === 'alert' ? 'bg-feedback-danger' : 'bg-feedback-success'
        }`}
      />
    </span>
  );
